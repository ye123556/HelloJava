...
public class ExCarTester3 {
    public static void main(String[] args) {
        ...
        // 다운 캐스팅
        // car4의 실제 인스턴스 ExCar, Car(상위) -> ExCar(하위) 캐스팅
        ExCar car5 = (ExCar) car4;

        ((ExCar)car4).getTotalMileage();

        // 명시적 형변환 불가능한 경우
        // 강제 타입 변환은 실제 인스턴스 부모인 경우엔 자식이 메모리에 없기 때문에 ClassCasetException 발생
        // 자식의 인스턴스 타입에 따라 결정됨
        ...
        // ClassCasetException 발생
        ExCar car7 = (ExCar) car6;

        System.out.println(car7.getTotalMileage());

        // 컴파일러 에러 발생, 수동으로 형변환 필요. 그러나 형변환 ClassCasetException 발생
