...
public class InstanceOfExample {
    ...
    // 다운 캐스팅 하기 전에 실제 instance를 확인하는 작업일 필요
    public static void main(String[] args) {
        Parent parentA = new Child();
        method1(parentA);
        method2(parentA);

        System.out.println("parentA는 Child가 맞나요? : " + (parentA instanceof Child));
        System.out.println("parentA는 Parent 인스턴스가 맞나요? : " + (parentA instanceof Parent));

        Parent parentB = new Parent();

        System.out.println("parentB는 Child가 맞나요? : " + (parentB instanceof Child));
